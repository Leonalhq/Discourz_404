...
def discussion_directory_path(instance, filename):
    # file will be uploaded to MEDIA_ROOT/user_<id>/<filename>
    return 'discussion/dis_{0}/{1}'.format(instance.id, filename)
...
class Discussion(models.Model):
    id = models.UUIDField(
        primary_key=True,
        default=uuid.uuid4,
        help_text="Unique ID for this Discussion",
    )
    isOpen = models.BooleanField(default=True)
    tags = models.CharField(max_length=100, default='["General"]')
    title = models.CharField(max_length=500, default='')
    initial_user = models.ForeignKey(User,on_delete=models.CASCADE,null=True,)
    date = models.DateField(default=datetime.now)
    imgUrl = "static/img/default.jpg"
    img = models.ImageField(upload_to=discussion_directory_path, default=imgUrl)
...
class Chat(models.Model):
    message = models.TextField(default='',primary_key=True)
    user = models.ForeignKey(User, on_delete=models.CASCADE, null=True)
